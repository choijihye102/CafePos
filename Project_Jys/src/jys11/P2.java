...
public class P2 implements ActionListener {
	...
	private int used_point = 0; // 추가 Used
	private int orderNum;
	private int currentMileage = 0; // 사용할 수 있는 마일리지.
	private int requestedMileage = 0; // 사용한 마일리지
	...
	private void initialize() { // Initialize the contents of the frame.
		...
		String[] header2 = { "총 금 액", "현재 보유 마일리지" };
		...
		spinner.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (selectedRow >= 0 && spinner.getValue() instanceof Integer) {
					...
					System.out.println("선택된 행이 없거나 잘못된 입력입니다.");
				}
			}
		});
	...
	private void updateTotalNum() { // 마지막시도 업데이트 이전.롤백지점.
		totalNum = 0; // 총합 초기화
		...
		DefaultTableModel newModel2 = new DefaultTableModel(new String[] { "총 금 액", "현재 보유 마일리지" }, 0);
		newModel2.addRow(new Object[] { totalPrice, currentMileage });
	...
	private void handleCustomerMileage() {
		...
		try {// 고객 존재 여부 확인
			...
			if (useMileageOption == JOptionPane.YES_OPTION) {
				...
				try {
					...
					if (mileage > currentMileage) {
						...
						finalPrice = totalPrice - requestedMileage; // 최종 결제 금액 계산
					}

					DefaultTableModel model2 = (DefaultTableModel) table2.getModel(); // 마일리지 사용 UI
					model2.setRowCount(0); // 기존 데이터 제거
					model2.addRow(new Object[] { totalPrice, requestedMileage });

					int rowsUpdated = dao.updateCus_point(cus_phon, requestedMileage); // 사용한 마일리지만큼 감소
					if (rowsUpdated > 0) {
						JOptionPane.showMessageDialog(frame,
								"마일리지 " + requestedMileage + "점이 사용으로 설정되었습니다. 최종 결제 금액: " + finalPrice + "원");
					} else {
						JOptionPane.showMessageDialog(frame, "마일리지 업데이트 중 오류가 발생했습니다.");
					}
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(frame, " SQLException e 발생 캐치문.");
		}
	}
	...
	private void handleCardPayment() { // 이제 mirotic 추가할거임.
		...
		try {
			...
			System.out.println("handleCardPayment - requestedMileage: " + requestedMileage); // 디버깅 used_point
			System.out.println("handleCardPayment - used_point: " + order.getUsed_point());

			order.setUsed_point(requestedMileage); // 추가사항 사용한 마일리지.

			int earnedMileage = 0; // 마일리지 적립 로직

			dao.insertOrderByPhon(order, cus_phon); // OracleSQL에 주문 추가
			JOptionPane.showMessageDialog(frame, "카드 결제가 완료되었습니다!");

			if (requestedMileage > 0) { // 마일리지 사용 여부에 따라 `used_point` 업데이트
				JOptionPane.showMessageDialog(frame, "사용된 마일리지: " + requestedMileage + "점");
			} else {
				earnedMileage = (int) Math.round(totalPrice * 0.05);
				dao.updateCus_point(cus_phon, earnedMileage); // 적립된 마일리지 추가
				JOptionPane.showMessageDialog(frame, "5% 적립되었습니다! 적립 마일리지: " + earnedMileage + "점");
			}
			
			// 4. Mirotic 테이블에 데이터 추가
	        int orderNum = dao.getOrderNumByPhon(cus_phon); // 방금 생성된 order_num 가져오기
	        for (int i = 0; i < model.getRowCount(); i++) {
	            String productName = (String) model.getValueAt(i, 0); // 제품명 가져오기
	            int quantity = (int) model.getValueAt(i, 2); // 수량 가져오기
	            ProductDTO product = dao.getProductByName(productName); // 제품 정보 가져오기

	            MiroticDTO mirotic = new MiroticDTO();
	            mirotic.setOrder_num(orderNum); // 주문 번호 설정
	            mirotic.setPro_id(product.getPro_id()); // 제품 ID 설정
	            mirotic.setQuantity(quantity); // 수량 설정

	            dao.insertMirotic(mirotic); // Mirotic 테이블에 데이터 삽입
	        }

	        JOptionPane.showMessageDialog(frame, "Mirotic 테이블에 데이터가 추가되었습니다!");
