...
import jys3.ProductDTO;
import jys3.CustomerDTO;
import jys3.OrdersDTO;
import jys3.MiroticDTO;
...
public class CafeDAO {
	...
	public int updateCus_point(String cus_phon, int cus_point) throws SQLException {	// 마일리지 업데이트 이후.
	    String sql = "UPDATE customer SET cus_point = cus_point - ? WHERE cus_phon = ?";
	    try (Connection conn = getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
	        pstmt.setInt(1, cus_point); // 감소할 마일리지 설정
	        pstmt.setString(2, cus_phon); // 업데이트 대상 고객 설정
	...
	public void insertOrderByPhon(OrdersDTO order, String cus_phon) throws SQLException {  // 롤백지점 usedPoint 값 롤백.this
